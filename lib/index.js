...
const dbUtils = {
		...
		return new Promise((resolve, reject) => {
			...
			pg.connect(conString, (err, client, done) => {
				try {
					clearTimeout(timer);
					if (err) {
						logError(err, `connect to connection pool for database "${hostAndDb.database}" on host "${hostAndDb.host}" failed.`);
						reject(err);
					}
					else {
	...
	close: (client, err) => {
		...
		if (client.releaseClient) {
			// passing err will destroy client rather than returning client to pool.  this ensures that a client is not returned back to pool in the middle of a transaction.
			if (err) {
